#!/bin/bash
    ...
    echo -e "$GREEN Installation of Gyuanx service node and Gyuanxd $COL_RESET"
    echo -e "$GREEN Everything will be compile for you $COL_RESET"
    echo -e "$GREEN************************************************************************$COL_RESET"
    echo
    sleep 8
...
export PATH=~/cmake-3.19.2-Linux-x86_64/bin:$PATH
cd
echo 'export PATH=~/cmake-3.19.2-Linux-x86_64/bin:$PATH' >> ~/.bashrc
...
echo 'export  BOOST_ROOT=/usr/local/include/boost' >> ~/.bashrc
source .bashrc
...
echo 'export OPENSSL_ROOT_DIR=/usr/local/openssl-${OPENSSL_VERSION}' >> ~/.bashrc
source .bashrc
cd
...
echo -n "Installation of the  gyuanx core begins"
    ...
    echo
    echo -e "$GREEN************************************************************************$COL_RESET"
    echo -e "$GREEN Service Node successfully Installed in your PC$COL_RESET"
    echo -e "$GREEN Installation of the  gyuanx core begins $COL_RESET"
    echo -e "$GREEN************************************************************************$COL_RESET"
    echo
    sleep 6
...
cmake .. -DBUILD_STATIC_DEPS=ON -DCMAKE_C_COMPILER=gcc-8 -DCMAKE_CXX_COMPILER=g++-8
...
sudo make install 
    ...
    echo
    echo -e "$GREEN*********************************************************************************************************************$COL_RESET"
    echo -e "$GREEN Service node and Gyuanx wallet successfully installed $COL_RESET"
    echo -e "$RED Run the service node with: gyuanx-storage your-ip-address  8080  --lmq-port 11111 $COL_RESET"
    echo -e "$RED Run the gyuanx node with: gyuanxd --service-node --storage-server-port 11115 --service-node-public-ip your-ip-address $COL_RESET"
    echo -e "$GREEN*********************************************************************************************************************$COL_RESET"
    echo
